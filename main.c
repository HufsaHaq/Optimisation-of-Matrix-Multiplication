...
#define METHOD_COMBINED  5  // Task Part II
...
// Task 1.6: inner block computation, improves cache reuse
            ...
            for (int sj = 0; sj < N; sj += BLOCK_SIZE) {
                ...
                for (int i = si; i < i_end; i++) {
                    for (int k = sk; k < k_end; k++) {
                        ...
                        // handle multiples of MM256_STRIDE first
                        ...
                        for (int j = sj; j < j_vec_end; j += MM256_STRIDE) {
                            ...
                            c = _mm256_add_pd(c, _mm256_mul_pd(b, a_ik));
...
int main(int argc, char **argv) {
    ...
    // allocate C (LxN), zero-initialised
